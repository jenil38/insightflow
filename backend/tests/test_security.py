...
def _register_and_login(client, email="user@example.com", password="StrongPass1!"):
    client.post("/auth/register", json={"email": email, "password": password, "full_name": "Test"})
...
class TestCrossTenantIsolation:
    ...
    def test_user_cannot_access_other_users_dataset_get(self, client, two_users, path_template):
    ...
    def test_user_cannot_access_other_users_dataset_post(self, client, two_users, path_template, body):
        _, bob_headers, dataset_id = two_users
        path = path_template.format(id=dataset_id)
...
class TestXlsRejected:
    def test_xls_upload_rejected(self, client):
        ...
        r = client.post(
            "/datasets/upload",
            headers=headers,
            files={"file": ("data.xls", io.BytesIO(b"fake xls"), "application/vnd.ms-excel")},
        )
