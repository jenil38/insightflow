...
SCORE_WEIGHTS = {"completeness": 0.35, "duplicates": 0.30, "consistency": 0.20, "uniqueness": 0.15}
...
STRONG_TARGET_KEYWORDS = (
    "revenue", "sales", "profit", "churn", "target", "label", "outcome",
    "converted", "conversion", "default", "fraud", "survived",
)
WEAK_TARGET_KEYWORDS = (
    "price", "amount", "total", "score", "rating", "value", "cost", "spend", "count",
)
...
def _looks_like_identifier(column: str, distinct: int, non_null: int, series: pd.Series) -> bool:
...
def recommend_targets(df: pd.DataFrame, limit: int = 5) -> list[dict[str, Any]]:
    ...
    for col in df.columns:
        ...
        if semantic == "numeric":
            ...
            if 2 <= distinct <= 15:
                confidence += 0.15
                reasons.append(f"it has {distinct} distinct classes, suitable for classification")
            else:
                confidence -= 0.10
                reasons.append(f"it has {distinct} classes, which is many for classification")
        ...
        candidates.append({
            ...
            "reason": ("Suggested because " + ", and ".join(reasons) + ".") if reasons else "Usable as a target.",
        })
...
def profile_column(df: pd.DataFrame, column: str, histogram_bins: int = 12) -> dict[str, Any]:
    series = df[column]
    ...
    profile: dict[str, Any] = {
        ...
        "unique_pct": safe_float(non_null_series.nunique() / non_null * 100, 2) if non_null else 0.0,
        ...
        "q1": None, "q3": None, "outlier_count": None,
        "top_values": [],
        "histogram": [],
    }
    ...
    if semantic == "numeric" and pd.api.types.is_numeric_dtype(series):
        ...
        parsed = pd.to_datetime(non_null_series, errors="coerce", format="mixed").dropna()
        if not parsed.empty:
...
def _consistency_score(df: pd.DataFrame) -> float:
    ...
    for col in df.columns:
        ...
        try:
            date_share = float(pd.to_datetime(sample, errors="coerce", format="mixed").notna().mean())
        except (ValueError, TypeError):
            date_share = 0.0
...
def compute_correlations(df: pd.DataFrame, max_columns: int | None = None) -> dict[str, Any]:
    ...
    if truncated:
        ...
        keep = numeric.var(numeric_only=True).sort_values(ascending=False).head(max_columns).index
        numeric = numeric[keep]

    corr = numeric.corr(numeric_only=True)
    columns = [str(c) for c in corr.columns]
    matrix = [[safe_float(corr.iloc[i, j], 3) for j in range(len(columns))] for i in range(len(columns))]
    ...
    return {"columns": columns, "matrix": matrix, "top_pairs": pairs[:12], "truncated": truncated}
...
def build_warnings(
    ...
    def add(severity: str, code: str, message: str, columns: list[str] | None = None):
        out.append({"severity": severity, "code": code, "message": message, "columns": columns or []})
    ...
    high_missing = [c["column"] for c in column_profiles if (c["missing_pct"] or 0) >= 40]
    if high_missing:
        add("danger", "high_missing", f"{len(high_missing)} column(s) are at least 40% empty.", high_missing)

    some_missing = [c["column"] for c in column_profiles if 0 < (c["missing_pct"] or 0) < 40]
    if some_missing:
        add("warning", "some_missing", f"{len(some_missing)} column(s) have some missing values.", some_missing)

    constant = [c["column"] for c in column_profiles if c["is_constant"]]
    if constant:
        add("warning", "constant_columns", f"{len(constant)} column(s) hold a single value and carry no signal.", constant)

    near_constant = [
        c["column"] for c in column_profiles
        if not c["is_constant"] and c["top_values"] and (c["top_values"][0]["pct"] or 0) >= NEAR_CONSTANT_RATIO * 100
    ]
    if near_constant:
        add("info", "near_constant_columns", f"{len(near_constant)} column(s) are over 95% a single value.", near_constant)

    high_cardinality = [
        c["column"] for c in column_profiles
        if c["semantic_type"] in ("categorical", "text") and (c["unique_pct"] or 0) > HIGH_CARDINALITY_RATIO * 100
    ]
    if high_cardinality:
        add("info", "high_cardinality", f"{len(high_cardinality)} text column(s) are near-unique per row.", high_cardinality)
    ...
    if identifiers:
        add("info", "identifier_columns", f"{len(identifiers)} likely identifier column(s) - exclude these from training.", identifiers)

    outlier_cols = [c["column"] for c in column_profiles if (c["outlier_count"] or 0) > 0]
    if outlier_cols:
        total = sum(c["outlier_count"] or 0 for c in column_profiles)
        add("info", "outliers", f"{total} outlier value(s) across {len(outlier_cols)} numeric column(s), by the 1.5xIQR rule.", outlier_cols)

    if scores["consistency_score"] < 85:
        add("warning", "mixed_types", f"Type consistency is {scores['consistency_score']}% - some text columns mix numbers, dates, and text.")

    return out


def build_recommendations(warnings_list: list[dict[str, Any]], scores: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if "duplicate_rows" in codes:
        recs.append({"action": "Remove duplicate rows", "where": "Data Quality", "why": "Duplicates bias every aggregate and inflate model scores."})
    if "high_missing" in codes:
        recs.append({"action": "Drop or impute the mostly-empty columns", "where": "Data Quality", "why": "Columns over 40% empty rarely help a model and can mislead imputation."})
    if "some_missing" in codes:
        recs.append({"action": "Choose a missing-value strategy", "where": "Data Quality", "why": "Explicit imputation beats letting each tool decide for itself."})
    if "constant_columns" in codes:
        recs.append({"action": "Exclude constant columns from training", "where": "Models", "why": "A column with one value cannot explain variation in the target."})
    if "identifier_columns" in codes:
        recs.append({"action": "Exclude identifier columns from training", "where": "Models", "why": "Row IDs leak row identity and produce unrealistically high scores."})
    if "mixed_types" in codes:
        recs.append({"action": "Parse dates and coerce numeric text columns", "where": "Data Quality", "why": "Mixed-type columns get treated as unordered text, losing their meaning."})
    if "outliers" in codes:
        recs.append({"action": "Review outliers before capping them", "where": "Data Explorer", "why": "Outliers are sometimes the most important rows, not errors."})

    if scores["overall_score"] >= 90 and not recs:
        recs.append({"action": "Train a model", "where": "Models", "why": "Data quality is already high enough to model directly."})
    return recs
...
def profile_dataframe(df: pd.DataFrame, size_bytes: int) -> dict[str, Any]:
    ...
    categorical_cols = [
        ...
        if str(c) not in numeric_cols and str(c) not in bool_cols and str(c) not in date_cols
    ]
    ...
    for c in numeric_cols:
        outlier_count += iqr_outlier_count(pd.to_numeric(df[c], errors="coerce").dropna())
...
class ProfilingService:
    ...
    def full_profile(
        ...
        for prof in column_profiles:
            type_counts[prof["semantic_type"]] = type_counts.get(prof["semantic_type"], 0) + 1

        numeric_summary = [
            {
                "column": p["column"], "min": p["min"], "max": p["max"], "mean": p["mean"],
                "median": p["median"], "std": p["std"], "missing_pct": p["missing_pct"],
                "outlier_count": p["outlier_count"],
            }
            for p in column_profiles if p["semantic_type"] == "numeric"
        ]
        categorical_summary = [
            {
                "column": p["column"], "unique_count": p["unique_count"],
                "missing_pct": p["missing_pct"], "top_values": p["top_values"][:5],
            }
            for p in column_profiles if p["semantic_type"] in ("categorical", "boolean", "text")
        ]
        ...
        return to_jsonable({
            "summary": {
                **profile_dataframe(df, size_bytes),
                "data_source": source,
                "sampled": sampled,
                "rows_in_file": total_rows if total_rows is not None else int(len(df)),
                "rows_analysed": int(len(df)),
            },
            "quality": {**scores, "grade": grade_for(scores["overall_score"])},
            "column_type_distribution": [{"type": k, "count": v} for k, v in sorted(type_counts.items())],
            "missing_by_column": [
                {"column": p["column"], "missing_count": p["missing_count"], "missing_pct": p["missing_pct"]}
                for p in sorted(column_profiles, key=lambda x: x["missing_pct"] or 0, reverse=True)
                if (p["missing_pct"] or 0) > 0
            ],
            "outliers_by_column": [
                {"column": p["column"], "outlier_count": p["outlier_count"]}
                for p in column_profiles if (p["outlier_count"] or 0) > 0
            ],
            ...
            "data_dictionary": [
                {
                    "column": p["column"], "dtype": p["dtype"], "semantic_type": p["semantic_type"],
                    "non_null_count": p["non_null_count"], "missing_pct": p["missing_pct"],
                    "unique_count": p["unique_count"], "example_values": p["example_values"],
                }
    ...
    def columns_overview(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        ...
        for col in df.columns:
            ...
            out.append({
                ...
                "missing_pct": safe_float((total - non_null) / total * 100, 2) if total else 0.0,
